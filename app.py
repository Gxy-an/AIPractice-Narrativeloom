...
from i18n import T, normalize_persona_pool
...
from llm_unified import cfg_from_preset, default_api_key_from_env, default_preset_from_env, verify_connection
...
from story_rag import build_chunks_from_beats, canon_sheet_from_beats, retrieve_context
...
def _extract_setting_baseline(outline: str) -> str:
    ...
    from display_utils import parse_merge_role_sections
...
def _parallel_roles_for_generation(beat_idx: int, lg: str) -> List[Tuple[str, str]]:
    """并行 LLM 生成（不含反套路创意师）。"""
    return [
        (r, t) for r, t in _active_functional_roles(beat_idx, lg) if not is_antitrope_role(r, lg)
    ]
...
def _run_auto_expand_all(llm_cfg: Dict[str, Any], lg: str, n: int) -> None:
    ...
    if outline_override:
        assembled_full = outline_override
        body_in = outline_override
    else:
        assembled_full = "\n\n".join(
            f"### {h} {i+1}\n{_beat_to_text(st.session_state.beats[i], lg)}"
            for i in range(n)
            if st.session_state.beats[i]
        )
...
def _render_antitrope_workflow(llm_cfg: Dict[str, Any], lg: str, n: int) -> None:
    ...
    full_outline = strip_assembly_beat_headers(assembled, h) or assembled.strip()
        ...
        if st.button(
            ...
            if variants:
                ...
                if 0 <= plan_i < len(variants):
                    ...
                    if outline:
                        st.session_state.story_outline_for_expand = strip_mutation_markers(outline)
            st.session_state.workflow_phase = "done"
            try:
                with st.spinner(T("auto_expand_running", lg)):
                    _run_auto_expand_all(llm_cfg, lg, n)
...
def _workspace(llm_cfg: Dict[str, Any]) -> None:
    ...
    with pane:
        ...
        if pool == "function":
            ...
            role_names = [nm for nm in role_order if not is_antitrope_role(nm, lg)]
            st.multiselect(
                T("roles_merge", lg),
                role_names,
                key=f"fn_multi_{idx}",
        ...
        if not (st.session_state.seed or "").strip():
            ...
            if _process_pending_beat_confirm(
                idx=idx,
                n=n,
                labels=labels,
                lg=lg,
            ...
            if need_gen or auto_gen:
                ...
                if st.button(T("confirm_section", lg), type="primary", key=f"ok_typ_{idx}"):
                    if not key_events_meaningful(ev):
                ...
                chosen_roles = [
                    r
                    for r in _sort_roles(list(st.session_state.get(f"fn_multi_{idx}", role_order)))
                    if not is_antitrope_role(r, lg)
                ]
                ...
                if default_outline and not (st.session_state.get(f"merge_preview_{idx}") or "").strip():
                    st.session_state[f"merge_preview_{idx}"] = default_outline
    ...
    if assembled.strip():
        if not (st.session_state.get("draft_asm") or "").strip():
            st.session_state["draft_asm"] = assembled
    ...
    if st.button(T("expand", lg)):
        ...
        body = outline_override or (st.session_state.get("draft_asm") or "").strip()
        if not body:
            ...
            body_in = body if outline_override else strip_assembly_beat_headers(body, hw)
            with st.spinner("…"):
                try:
